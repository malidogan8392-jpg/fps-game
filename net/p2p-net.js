...
// ── _mainSocket: index.html'in kullandığı tek nesne ───────────────────
class P2PSocket {
    constructor() {
        this._handlers = {};
...
async function p2pNetLogin({ user, pwd }) {
    try {
        ...
        persistSession();
        _mainSocket._fire('authOk', { username: myUsername });
    } catch (e) { _mainSocket._fire('authErr', e.message); }
}
...
async function p2pNetGetRooms() {
    try {
        ...
        _mainSocket._fire('roomList', list);
    } catch (e) { _mainSocket._fire('roomList', []); }
}

async function p2pNetQuickJoin() {
    try {
        const data = await hubFetch('/api/servers/quick');
        if (data.server) becomeClient(_mainSocket, data.server.peerId);
        else becomeHost(_mainSocket, 'public', 'Hızlı Sunucu');
    } catch (e) {
        // Hub'a ulaşılamıyorsa yine de kendi P2P sunucunu açıp oynayabilesin
        becomeHost(_mainSocket, 'public', 'Hızlı Sunucu (çevrimdışı hub)');
    }
}

function p2pNetJoinRoom(id) { becomeClient(_mainSocket, id); }
