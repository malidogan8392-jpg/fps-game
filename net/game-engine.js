// ── P2P Oyun Motoru (eski server.js'in host tarafında çalışan hali) ──────
// Bu dosya, host olan oyuncunun tarayıcısında çalışır. Eskiden Node.js
// sunucusunda (server.js) çalışan oda/hasar/mağaza/oy mantığının aynısı —
// tek fark: artık socket.io yerine p2p-net.js'in sağladığı "FakeIO" üzerinden
// çalışıyor, ve hesaplar burada YOK (hesaplar Hub backend'i + her oyuncunun
// kendi tarayıcısı tarafından yönetiliyor; bkz. p2p-net.js).
//
// Tek oda var (bir host = bir oda), bu yüzden orijinal server.js'teki
// çoklu-oda (rooms{}) yapısı kaldırıldı ama io.to(rid)/socket.to(rid) gibi
// çağrılar aynen duruyor (FakeIO bunları tek odaya broadcast olarak uygular).
...
window.MAPS = {
    ...
    castle:  { name: 'Kale', theme: 'grass', size: 40, model: { url: 'public/models/castle.glb', scale: 0.13293, rotY: 0, offsetX: 1.739, offsetY: 7.872, offsetZ: 0.828 } },
    castle_arena:  { name: 'Kale Arenası', theme: 'grass', size: 100, model: { url: 'public/models/fps_map_warface_big_castle_pvp_pve.glb', scale: 0.98981, rotY: 0, offsetX: 20.701, offsetY: 3.852, offsetZ: 23.136 } },
    neon_grand:    { name: 'Neon Kompleks', theme: 'urban', size: 110, model: { url: 'public/models/a_very_beautiful_neon_fps_pvp_map.glb', scale: 0.93519, rotY: 0, offsetY: 6.944 } },
    warehouse:     { name: 'Depo', theme: 'industrial', size: 65, model: { url: 'public/models/fps_pvp_map_and_pve.glb', scale: 1.07602, rotY: 0, offsetX: 13.953, offsetY: -1.888, offsetZ: -12.66 } },
    district:      { name: 'Bölge', theme: 'urban', size: 55, model: { url: 'public/models/pvp_map_xxxl_big_fps_pve.glb', scale: 0.77984, rotY: 0, offsetY: 1.312 } },
    outpost:       { name: 'Karakol', theme: 'urban', size: 24, model: { url: 'public/models/fps_map.glb', scale: 0.94248, rotY: 0, offsetY: 0 } },
    outpost_neon:  { name: 'Neon Karakol', theme: 'urban', size: 24, model: { url: 'public/models/fps_map_pvppve_neon.glb', scale: 1.01502, rotY: 0, offsetY: 0.984 } },
    complex:       { name: 'Kompleks', theme: 'industrial', size: 40, model: { url: 'public/models/fps_map_pvp_pve_games.glb', scale: 0.94994, rotY: 0, offsetY: 0 } },
    facility:      { name: 'Tesis', theme: 'industrial', size: 38, model: { url: 'public/models/pvp_map_fps.glb', scale: 0.48515, rotY: 0, offsetY: 3.472 } },
    shatterline:   { name: 'Shatterline', theme: 'industrial', size: 18, model: { url: 'public/models/fps_map_pvp_shatterline_neon_3x3.glb', scale: 0.94719, rotY: 0, offsetY: 0 } },
    neon_arena:    { name: 'Neon Arena', theme: 'urban', size: 22, model: { url: 'public/models/fps_map_pvp_pve_game_neon.glb', scale: 0.47812, rotY: 0, offsetY: 1.442 } },
    ruins:         { name: 'Yıkıntı', theme: 'industrial', size: 30, model: { url: 'public/models/fps_warface_map_destruction_pvp_shuret_neon.glb', scale: 0.89549, rotY: 0, offsetY: 0 } },
    freezone:      { name: 'Serbest Bölge', theme: 'urban', size: 22, model: { url: 'public/models/free_map_pvp_fps_neon.glb', scale: 0.90308, rotY: 0, offsetY: -0.526 } },
    sector:        { name: 'Sektör', theme: 'urban', size: 18, model: { url: 'public/models/fps_warface_map_neon_pvp_pve.glb', scale: 0.70686, rotY: 0, offsetY: 0 } },
    crossing:      { name: 'Kavşak', theme: 'urban', size: 70, model: { url: 'public/models/city_scene.glb', scale: 0.00545, rotY: 0, offsetX: 2.863, offsetY: -0.313, offsetZ: 0.088 } },
    labyrinth:     { name: 'Labirent', theme: 'industrial', size: 45, model: { url: 'public/models/labyrinth.glb', scale: 0.63068, rotY: 0, offsetX: 2.045, offsetY: 6.307, offsetZ: -1.023 } },
    warship:       { name: 'Savaş Gemisi', theme: 'harbor', size: 50, model: { url: 'public/models/warship.glb', scale: 0.01888, rotY: 0, offsetX: -1.797, offsetY: 0.008, offsetZ: 0.2 } }
};
...
// Host bu fonksiyonu bir kere çağırır; io, p2p-net.js'in ürettiği FakeIO'dur.
function createRoomEngine(io, opts) {
    ...
    // Diğer koda (lobi/hub heartbeat) oda durumunu okutmak için dışa açık referans
    io._room = r;
    ...
    // Bağlantısı sessizce kopan (ghost) oyuncuları temizlemek için canlılık takibi
    const LIVENESS_TIMEOUT = 12000;
    const livenessInterval = setInterval(() => {
        const now = Date.now();
        Object.keys(r.players).forEach((pid) => {
            const p = r.players[pid];
            if (p._lastSeen && now - p._lastSeen > LIVENESS_TIMEOUT) {
                delete r.players[pid];
                io.to(RID).emit('playerLeft', pid);
            }
        });
    }, 4000);
    io._onDestroy = () => clearInterval(livenessInterval);
    ...
    io.on('connection', (socket) => {
        ...
        socket.on('helloJoin', (hello) => { enterRoom(hello); if (r.players[socket.id]) r.players[socket.id]._lastSeen = Date.now(); });

        socket.on('_ping', () => { const p = r.players[socket.id]; if (p) p._lastSeen = Date.now(); });
